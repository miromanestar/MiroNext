...
const Youtube = () => {
  const youtubeRan = useRef(false);

  useEffect(() => {
    if (youtubeRan.current) return;

    new YTGallery(
      "PL4BrNFx1j7E6a6IKg8N0IgnkoamHlCHWa",
      "LOL_NO_KEY",
      "gallery-showcase",
    );
    youtubeRan.current = true;
  }, []);
...
const YTGallery = class {
  constructor(id, key, el, opts = {}) {
    ...
    if (!(id && key && el)) {
      ...
      if (!id) errMsg = "No playlist ID set";
      if (!key) errMsg = "No API key set";
      if (!el) errMsg = "No container element ID set";
    ...
    this.cache = localStorage[this.cacheName]
      ? JSON.parse(localStorage[this.cacheName])
      : null;
    this.playlistInfo = this.cache
      ? this.cache.playlistInfo
    ...
    this.eye =
      '<svg class="ytgallery-meta-icon" xmlns="http://www.w3.org/2000/svg" width="1792" height="1792" viewBox="0 0 1792 1792"><path d="M1664 960q-152-236-381-353 61 104 61 225 0 185-131.5 316.5t-316.5 131.5-316.5-131.5-131.5-316.5q0-121 61-225-229 117-381 353 133 205 333.5 326.5t434.5 121.5 434.5-121.5 333.5-326.5zm-720-384q0-20-14-34t-34-14q-125 0-214.5 89.5t-89.5 214.5q0 20 14 34t34 14 34-14 14-34q0-86 61-147t147-61q20 0 34-14t14-34zm848 384q0 34-20 69-140 230-376.5 368.5t-499.5 138.5-499.5-139-376.5-368q-20-35-20-69t20-69q140-229 376.5-368t499.5-139 499.5 139 376.5 368q20 35 20 69z"/></svg>';
    this.clock =
  ...
  checkCache() {
    ...
    if (time - this.cache.time > this.cacheLife) {
      this.getPlaylistData();
      console.log(
        `Cache for \"${this.cacheName}\" is more than a day old... building`,
      );
    } else {
      ...
      console.log(
        `Cache for \"${this.cacheName}\" is les than a day old... using cache`,
      );
    }
  }
  ...
  async getPlaylistData(data, token) {
    ...
    let url = this.buildUrl("/api/playlistItems", {
    ...
    const res = await fetch(url).catch((error) => {
      this.handleError(
        error,
        "A problem occured while retrieving data from Youtube.",
      );
    ...
    for (const item of data.items) {
      if (item.snippet.title !== "Private video")
        ...
        console.warn(
          `Video with ID \"${item.snippet.resourceId.videoId}\" is private... skipping.`,
        );
    }
    ...
    if (data.nextPageToken) {
      ...
      console.log(
        `Playlist items successfully grabbed with ${videoIds.length} items... grabbing item data.`,
      );
  ...
  async buildCache(videoIds, data, iteration = 0) {
    ...
    let url = this.buildUrl("/api/videos", {
      id: ids.toString(),
    });
    ...
    for (const item of data.items) {
      this.cache.pages[i - 1].push({
        ...
        duration: this.parseIsoToDuration(
  ...
  async getPlaylistInfo() {
    let url = this.buildUrl("/api/playlists", {
      id: this.playlistId,
    });
  ...
  insert(el, name, parent) {
    ...
    if (!parent) this.elems.container.appendChild(temp);
  ...
  paginate(dir) {
    if (this.okayToPaginate) {
      if (
        (dir === "Back" && this.currentPage === 1) ||
        (dir === "Next" && this.currentPage === this.cache.numPages)
      )
        return;
      ...
      if (
        (dir === "Back" && this.getPageData(this.currentPage - 2)) ||
        (dir === "Next" && this.getPageData(this.currentPage).length > 0)
      ) {
  ...
  handleError(error, msg) {
    ...
    if (this.elems) {
      ...
      if (this.searchEnabled) this.hide(this.elems.search);
    } else if (this.elId) {
